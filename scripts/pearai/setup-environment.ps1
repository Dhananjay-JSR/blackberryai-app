...
function Initialize-BaseFunctionality {
    ...
    Set-Location .\extensions\blackberryai-submodule
...
function Create-SymLink {
    Write-Host "`nCreating symbolic link 'extensions\blackberryai-ref' -> 'extensions\blackberryai-submodule\extensions\vscode'" -ForegroundColor White
...
$modulePath = Join-Path -Path $currentDir -ChildPath 'extensions\blackberryai-submodule'
$targetPath = Join-Path -Path $modulePath -ChildPath 'extensions\vscode'
$linkPath = Join-Path -Path $currentDir -ChildPath 'extensions\blackberryai-ref'
