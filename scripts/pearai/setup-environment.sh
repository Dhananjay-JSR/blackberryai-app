...
app_dir=$(pwd)
target_path="$app_dir/extensions/blackberryai-submodule/extensions/vscode"
link_path="$app_dir/extensions/blackberryai-ref"

# Run the base functionality
echo -e "\nInitializing sub-modules..."

# Check if the submodule directory already exists
if [ -d "$app_dir/extensions/blackberryai-submodule" ]; then
    echo "Removing existing blackberryai-submodule directory"
    execute "rm -rf $app_dir/extensions/blackberryai-submodule" "Failed to remove existing blackberryai-submodule directory"
fi
...
execute "cd ./extensions/blackberryai-submodule" "Failed to change directory to extensions/blackberryai-submodule"
